...
/**
 * Discover running OpenACP instances by reading the global registry
 * and checking each instance's api.port file + health endpoint.
 */
export async function discoverRunningInstances(
  ...
  for (const entry of Object.values(registry.instances)) {
    try {
      ...
      // api.port doesn't exist — instance not running
      continue
    }
  }
