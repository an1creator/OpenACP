...
- **Commands** — Agents can publish slash commands that appear as action buttons in the session topic. Tapping one sends the command to the current agent with exactly one leading `/`. If the command declares an input hint, OpenACP asks for the argument first. Typed OpenACP system commands keep precedence when a name overlaps; the agent button remains explicitly routed to the agent.
