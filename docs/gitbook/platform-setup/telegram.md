...
- **Agent actions** — OpenACP shows the supported subset of `compact`, `goal`, `logout`, `mcp`, `plan`, `review`, `review-branch`, `review-commit`, `skills`, and `status` as inline buttons pinned at the top of the topic. Per-skill `$name` buttons are omitted; with an attested official Codex adapter installation, `/skills` sends a completed names-only response immediately and `$name` still invokes the skill normally. `/skills` never opens a ForceReply input, finalizes an active response draft, or clears its tool progress. OpenACP replaces the complete pinned set before removing the previous one, resumes interrupted cleanup after restart, and clears stale buttons when a current snapshot is unavailable. Button actions are authorized against the exact live session, agent process, Telegram adapter, user, and topic after the security check; a detached or remapped topic receives no stale response. Normal actions preserve the agent's advertised slash and case. If one requests input, reply to its prompt; paths and other values beginning with `/` are accepted in that reply. Typed OpenACP commands such as `/status` and `/STATUS` keep their system meaning even if an agent advertises the same name.
